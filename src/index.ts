...
export default class Gmail {
    ...
    constructor() {
        ...
        this.TOKEN_PATH = './GoogleAPIs/Gmail/token.json';
    ...
    private authorize() {
        return new Promise((resolve, reject) => {
            fs.readFile(this.CREDENTIALS_PATH, (err, content) => {
                ...
                let credentials = JSON.parse(<string><unknown>content)
    ...
    private send(gmail, email, userId) {
    ...
    public setFrequency = function (freq) {
        this.sendFrequency = freq
        console.log('Reset sending frequency', freq)
    }
    public SingleDelivery(mail) {
        //  if(typeof mail==='json') throw "Invalid Types"
        let headers = this.Head + 'Content-Length: ' + mail.body.length + '\r\n\r\n'
        ...
        return new Promise((resolve, reject) => {
            ...
            this.authorize().then((auth) => {
                ...
                this.send(google.gmail({ version: 'v1', auth }), mail64, GmailConfig.userId)
                    .then((res) => {
    ...
    public DatasetDelivery(mail, content, database) {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                let headers = this.Head + 'Content-Length: ' + current_email.length + '\r\n\r\n'
    ...
    public DistributedCampaign = function (mail, content, database, options) {
        var MAX_DATA = 50
        var payload = []
